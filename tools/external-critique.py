...
def member_nvidia2(subject, web, key, model, cap=450):
    try:
        return nvidia([{"role": "system", "content": CRITIC_SYS},
                       {"role": "user", "content": f"{subject}\n\nЖИВОЙ ВЕБ:\n{web}"}],
                      key, model, max_tokens=cap)
    except Exception as e:
        if "deepseek" in model:  # фолбэк на быструю
            return member_nvidia2(subject, web, key, "meta/llama-3.3-70b-instruct", cap)
...
def main():
    ...
    ap.add_argument("--deep", action="store_true", help="добавить deepseek-r1 (глубже, но дороже по токенам)")
    ap.add_argument("--synth-only", action="store_true", help="печатать только синтез (экономия вывода)")
    args = ap.parse_args()
    env = load_env()
    ...
    # ponytail-режим: проси КОРОТКО — буллеты, без воды
    subject += "\n\nОТВЕЧАЙ КОРОТКО: только буллеты дефектов/рисков + 1-строчный вердикт. Без воды/повторов."

    web = serper_web(args.prompt[:120], serper)
    CAP = 800 if args.deep else 450   # lite по умолчанию = меньше токенов

    # СОВЕТ — LITE: 2 быстрые модели (llama 70b+8b). DEEP: + deepseek-r1 (reasoning, дорого).
    members = [("llama-3.3-70b", "meta/llama-3.3-70b-instruct"),
               ("llama-3.1-8b", "meta/llama-3.1-8b-instruct")]
    if args.deep:
        members.insert(0, ("deepseek-r1", "deepseek-ai/deepseek-r1"))
    with ThreadPoolExecutor(max_workers=len(members)+1) as ex:
        futs = [(n, ex.submit(member_nvidia2, subject, web, nv, m, CAP)) for n, m in members]
        f_gm = ex.submit(member_gemini, subject, gkey)
        panel = [(n, f.result()) for n, f in futs]
        gm = f_gm.result()
    ...
    # СИНТЕЗ — chairman сводит, КОРОТКО (быстрая 8b)
    joined = "\n\n".join(f"[{n}]\n{t}" for n, t in panel)
    try:
        synth = nvidia([
            {"role": "system", "content": "Ты chairman. КОРОТКО сведи советников: согласие=уверенно · "
             "расхождение=флаг · список правок по приоритету · финал ИДЕАЛЬНО / НУЖНЫ ПРАВКИ. Только суть."},
            {"role": "user", "content": f"Тема: {args.prompt}\n\nМНЕНИЯ:\n{joined}"}],
            nv, "meta/llama-3.1-8b-instruct", max_tokens=600, timeout=60)
        print(f"\n##### СИНТЕЗ #####\n{synth}")
    except Exception as e:
        print(f"\n(синтез недоступен: {e})")
    print(f"\n[совет: {len(panel)} модел{'и' if len(panel)<5 else 'ей'}{'+gemini' if gm else ''} · режим: {'DEEP' if args.deep else 'LITE'} · cap {CAP}т]")
