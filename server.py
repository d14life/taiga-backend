...
_BENCH = (
    ...
    ("gpt-4",                     (80, 80, 76,  0, 78)),
    ("claude-fable-5",            (99, 99, 98, 92, 93)),   # AA июнь-2026: #1 Intelligence (65) + лидер кодинга
    ("fable",                     (96, 96, 95, 90, 92)),
...
def bench(model_id: str, task: str = "general", name: str = "") -> float:
    ...
    if row is None:
        return -1.0
    if task == "agent":
        # Artificial Analysis: агентность (GDPval-AA / Terminal-Bench / SWE) ≈ кодинг + ризонинг +
        # следование инструкциям. Веса отражают акценты AA: код важнее всего, потом ризонинг, потом общий.
        return float(round(0.5 * row[1] + 0.3 * row[2] + 0.2 * row[0], 1))
...
BEAM_FUSION_PROMPT = (
    ...
    "Твоя задача — НЕ выбрать лучший и НЕ усреднить, а СПЛАВИТЬ их в ОДИН НОВЫЙ ответ, который "
    "СИЛЬНЕЕ и ПОЛНЕЕ любого исходного по отдельности. Действуй как судья-синтезатор по пяти осям:\n"
    "1) КОНСЕНСУС — где ответы согласны между собой: высокая уверенность, бери в основу.\n"
    "2) ПРОТИВОРЕЧИЯ — где расходятся: не усредняй, выбери версию, лучше всего обоснованную "
    "логикой и фактами; слабую/ошибочную отбрось.\n"
    "3) УНИКАЛЬНЫЕ ИНСАЙТЫ — ценное и ВЕРНОЕ, что заметила лишь одна модель: ОБЯЗАТЕЛЬНО включи. "
    "В этом сила ансамбля — собрать вместе то, чего нет ни в одном ответе по отдельности.\n"
    "4) ГАЛЛЮЦИНАЦИИ / СЛЕПЫЕ ЗОНЫ — то, что привела лишь одна модель, не подтверждается и "
    "выглядит сомнительно: отбрось, не включай.\n"
    "5) ПРОБЕЛЫ ОХВАТА — чего не хватает во ВСЕХ ответах для полноты: дополни сам, если уверен в факте.\n"
    "Итог — один связный, точный и более полный ответ на языке пользователя, чем каждый по отдельности. "
    "Не упоминай эти ответы, «модели», «советников», голосование или процесс сверки — "
...
def _fusion_agreement(heads) -> str:
    ...
    for mc in minority[:5]:
        out.append(f"  • только 1/{n} модель — реши по фактам/логике: либо УНИКАЛЬНЫЙ ИНСАЙТ (верно и ценно → включи), либо галлюцинация (выдумка → отбрось): {mc[:170]}")
    return "\n".join(out)
...
def detect_task(messages: list, has_images: bool) -> str:
    ...
    for mm in reversed(messages):
        if mm.get("role") == "user":
            ...
            break
    # АГЕНТНАЯ задача (AA-категория «agentic»: GDPval/Terminal-Bench) — многошаговая сборка/деплой/
    # оркестрация/рефактор. Ловим РАНЬШЕ простого кода: автомоду нужна сильная агентная модель в бюджете.
    if re.search(r"\b(собери|построй|развер|задепло|деплой|оркестр|пайплайн|воркфлоу|многоэтап|многошаг|рефактор|мигрир|end.?to.?end|агентск|по\s+шагам)\w*", last):
        return "agent"
    if "```" in last or re.search(r"\b(код|програм|функци|python|javascript|sql|регуляр|bug|debug|компил)\w*", last):
        return "code"
...
class Handler(BaseHTTPRequestHandler):
    ...
    def chat(self):
        ...
        # ДОЛГАЯ память юзера (авто-извлечённые факты) подмешивается ПО УМОЛЧАНИЮ — иначе ИИ
        # «не помнит» юзера в новом чате (баг рефлексии: факты сохранены в init.memory, но не
        # инжектились). Капы memory_block малы (6 фактов / 600 симв). Выключить — server_memory:false.
        if req.get("server_memory") is not False:
