...
ШАГ 0: прочитай docs/MANIFEST.md (законы + рельсы) и подними ТРОЙНУЮ память:
  ...
  D. ponytail (экономия токенов): /plugin marketplace add DietrichGebert/ponytail затем /ponytail full
  Дальше вместо грепанья/догадок — спрашивай память. Читай docs/REFERENCE-REPOS.md +
  docs/PARKED-FEATURES.md (отложенные фичи — не потерять). Потом docs/NEW-SESSION-START.md.
...
## 6. ИСТОРИЯ ЧАТОВ
- Сжатая (всё важное) — внутри `docs/PROJECT-BIBLE.md` (таймлайн + ключевые развилки).
- Сырые транскрипты (локально, ~460 MB каждый, в репо не кладём):
  `/Users/damir12/.claude/projects/-Users-damir12/*.jsonl` — самый большой `2ab72971-…jsonl` = текущая длинная сессия.
