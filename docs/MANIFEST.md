...
- **СТРУКТУРА = 5 СТОЛОВ + Ultra (решение Damir, DECISIONS-LOG):** Разговор(Чат)/Студия/Дизайн/Агенты/
  Код + Ultra. Моды (Мозг/Совет/Дебаты/Сравнить/Relay) = **«УСИЛЕНИЕ ответа»** (уровень поведения),
  НЕ отдельные экраны. **Источник истины каждого стола = его эталон-репо (REFERENCE-REPOS.md) +
  design-v3** — юзать ВСЕГДА как базу. Студия → база `anil-matcha/open-generative-ai`. Порядок сборки
  столов — в TEST-WORKFLOW. (Это перевешивает старые «8 фаз / Modes» из LOVABLE-HANDOFF.)
