...
- **ВНЕШНИЙ СОВЕТ + СИНТЕЗ (другой взгляд + живой веб, как «Совет» в Тайге).** Чтобы Claude не судил
  сам себя — на важных шестерёнках/кусках кода/доках требований зови:
  `python3 tools/external-critique.py "<вопрос>" [--file path]`. Совет НЕ-Claude моделей независимо
  критикует (NVIDIA deepseek-r1 + llama-3.3-70b — разные семейства; Gemini 2.5 + Google-grounding когда
  есть маршрут сервер/туннель), сверяясь с живым вебом (Serper, из РФ напрямую), потом chairman
  СИНТЕЗирует: согласие = уверенность, расхождения = флаг, список правок. Ключи — ~/.reel-intelligence.env.
  (Gemini из РФ только через рабочий выход — сервер/Молдова/Албания; без него совет идёт на NVIDIA.)
